...
class RemoteSimulator:
    ...
    def start(self) -> bool:
        ...
        if self.verbose:
            ...
            if (
                self._callCommand(GetSimulatorState()).subStateId()
                == SimulatorSubState.Started_HILSync
            ):
    ...
    def _checkForbiddenCall(self, cmd: CommandBase) -> None:
        ...
        if cmd.getName() == "PushRouteEcef":
            raise Exception(
                "You cannot call a PushRouteEcef command. Post it or use RemoteSimulator.pushRouteLla() or RemoteSimulator.pushRouteEcef() instead."
            )
        if cmd.getName() == "PushTrackEcef":
            raise Exception(
                "You cannot call a PushTrackEcef command. Post it or use RemoteSimulator.pushTrackEcef() or RemoteSimulator.pushTrackLla() instead."
            )
        if cmd.getName() == "PushTrackEcefNed":
            raise Exception(
                "You cannot call a PushTrackEcefNed command. Post it or use RemoteSimulator.pushTrackEcefNed() or RemoteSimulator.pushTrackLlaNed() instead."
            )
    ...
    def checkIfStreaming(self) -> bool:
        ...
        if stateResult.state() == "Error":
            ...
            errorMsg = (
                "Simulator is not running. Current state is "
                + stateResult.state()
                + "."
            )
    ...
    def waitState(self, state, failureState=""):
        self._checkConnect()
        if self.verbose:
            print("Waiting for simulator state " + str(state) + "...")
        ...
        if stateResult.state() == state:
            ...
            errorMsg = (
                "Wrong simulator state. Expected ",
                state,
                " but received ",
                stateResult.state(),
            )
    ...
    def _checkHil(self, pos, elapsedTime):
        if self.hil == None:
            raise Exception(
                "Cannot send positions to simulator because you are not connected."
            )
        ...
        if elapsedTime - self.checkRunningTime >= 1000.0:
            ...
            if self.hilStreamingCheckEnabled and not self.checkIfStreaming():
                ...
                return False
            if self.verbose:
                print(str(pos) + " sent at %2fms" % elapsedTime)
        return True
    ...
    def pushRouteEcef(self, speed, ecef):
        ...
        self._postCommand(PushRouteEcef(speed, ecef.x, ecef.y, ecef.z))
        return True
    ...
    def pushTrackEcef(self, elapsedTime, ecef):
        ...
        self._postCommand(PushTrackEcef(elapsedTime, ecef.x, ecef.y, ecef.z))
        return True
    ...
    def pushIntTxTrackEcef(self, elapsedTime, ecef, id):
        ...
        self._postCommand(PushIntTxTrackEcef(elapsedTime, ecef.x, ecef.y, ecef.z, id))
        return True
    ...
    def endIntTxTrackDefinition(self, id):
        ...
        if self.verbose:
            print(
                "End transmitter track contains " + str(trackResult.count()) + " nodes."
            )
