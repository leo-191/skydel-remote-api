...
class Lla:
    ...
    def toEcef(self) -> "Ecef":
        ...
        return Ecef(
            (n + self.alt) * cos_lat * math.cos(self.lon),
            (n + self.alt) * cos_lat * math.sin(self.lon),
            (tmp * n + self.alt) * math.sin(self.lat),
        )

    def toEnu(self, origin: "Lla"):
        return Enu(
            (self.lon - origin.lon) * ESMAJ * math.cos(self.lat),
            (self.lat - origin.lat) * ESMAJ,
            self.alt - origin.alt,
        )
    ...
    def __str__(self):
        return "%.7fdeg, %.7fdeg, %.2fm" % (self.latDeg(), self.lonDeg(), self.alt)
...
class Enu:
    ...
    def toEcef(self, originLla: Lla) -> "Ecef":
        ...
        x = (
            -sinLon * self.east
            - sinLat * cosLon * self.north
            + cosLat * cosLon * self.up
            + originEcef.x
        )
        y = (
            cosLon * self.east
            - sinLat * sinLon * self.north
            + cosLat * sinLon * self.up
            + originEcef.y
        )
...
class Ecef:
    ...
    def __str__(self):
        return "%.2fm, %.2fm, %.2fm" % (self.x, self.y, self.z)
